...
def post_optimization(
    ...
    for img_path in image_lists:
        os.system(f"cp -r {img_path} {osp.join(temp_image_path, osp.basename(img_path))}")
    ...
    for i in iter_id:
        ...
        if state == False:
            print(
                "Build colmap coarse dataset fail! colmap point3D or images or cameras is empty!"
            )
        ...
        if i == 0:
            ...
            if database_path is None:
                ...
                os.system(f"cp {database_path} {osp.join(colmap_refined_kpts_dir, 'database.db')}")
