...
import cv2
import sys
sys.path.append("./src/DKM")
from src.DKM.dkm import DKMv3_outdoor, DKMv3_indoor
...
def match_worker(subset_ids, image_lists, covis_pairs_out, cfgs, pba=None, verbose=True):
    ...
    if args['model']['matcher'] == "DKM":
        dkm_model = DKMv3_indoor(device=device)
        matches = {}
        for data in tqdm(dataloader):
            f_name0, f_name1 = data['pair_key'][0][0], data['pair_key'][1][0]
            data_c = {k: v.cuda() if isinstance(v, torch.Tensor) else v for k, v in data.items()}

            im1_path = data_c['pair_key'][0][0]
            im2_path = data_c['pair_key'][1][0]

            im1_H, im1_W = cv2.imread(im1_path).shape[:2]
            im2_H, im2_W = cv2.imread(im2_path).shape[:2]
                        
            # Match
            warp1, certainty1 = dkm_model.match(im1_path, im2_path, device=device)
            certainty1 = certainty1.squeeze()                
            # warp1, certainty1 = roma_model.match(im1_path, im2_path, device=device)
            
            nms_certainty1 = simple_nms(certainty1, nms_radius=2)
            # nms_certainty1 = certainty1
            
            dkm_matches = dkm_model.kpts_match(warp1, nms_certainty1, thres=0.2)
                        
            mkpts0, mkpts1 = dkm_model.to_pixel_coordinates(dkm_matches, im1_H, im1_W, im2_H, im2_W)
                
            quantize_ratio = 8
            if True:
                mkpts0[:,0] = ((mkpts0[:,0] / quantize_ratio).round() * quantize_ratio) #  * W_A / upsample_res_W
                mkpts0[:,1] = ((mkpts0[:,1] / quantize_ratio).round() * quantize_ratio) # * H_A / upsample_res_H
                
                mkpts1[:,0] = ((mkpts1[:,0] / quantize_ratio).round() * quantize_ratio) # * W_A / upsample_res_W
                mkpts1[:,1] = ((mkpts1[:,1] / quantize_ratio).round() * quantize_ratio) # * H_A / upsample_res_H
                
            else:
                mkpts0[:,0] = (mkpts0[:,0]) * W_A / upsample_res_W
                mkpts0[:,1] = (mkpts0[:,1]) * H_A / upsample_res_H
                
                mkpts1[:,0] = (mkpts1[:,0]) * W_A / upsample_res_W
                mkpts1[:,1] = (mkpts1[:,1]) * H_A / upsample_res_H

            mkpts0 = mkpts0.cpu().numpy()
            mkpts1 = mkpts1.cpu().numpy()

            mconfs = np.ones(mkpts0.shape[0])
        ...
        for data in tqdm(dataloader):
            ...
            mkpts0 = (mkpts0/8).round()*8
            mkpts1 = (mkpts1/8).round()*8
