...
from .utils.gp_matching import GPMatching
...
class MultiviewMatcher(nn.Module):
    def __init__(self, config, profiler=None, debug=False, test=False, plotting_vis=None, test_speed=False):
        ...
        # Misc
        self.config = config
        self.profiler = None # profiler or PassThroughProfiler()
        self.debug = debug
    ...
    def forward(self, data, chunk_track=1000, chunk_backbone_img=True):
        ...
        for step in range(self.n_steps):
            ...
            if self.training or not chunk_backbone_img:
                ...
                with nullcontext(): # self.profiler.record_function("MultiviewMatcher/fine-process"):
                ...
                # Extract features:
                with nullcontext(): # self.profiler.record_function("MultiviewMatcher/backbone"):
            ...
            for it in range(len(num_track_list)):
                ...
                s2d_ref_features, s2d_query_features = features_crop_reference_super_patch, features_crop_query[:, i : (i + num_track), :chunk_view-1]


                # Perform multiview transformer:
                with nullcontext(): # self.profiler.record_function("MultiviewMatcher/transformer"):
                    if self.test_speed:
                        torch.cuda.synchronize()
                        transform_t0 = time()
                ...
                with nullcontext(): # self.profiler.record_function("MultiviewMatcher/matching"):
                    if self.test_speed:
                        torch.cuda.synchronize()
                        matching_t0 = time()
                    ...
                    if  (self.training) or (current_reference_location.transpose(1, 2)[:, i : (i + num_track), :chunk_view-1].shape[2] > -1):                        
                        (
                            query_points_refined,
                            reference_points_refined,
                            conf,
                            ...
                            s2d_query_features,
                            data["query_points"][:, i : (i + num_track)],
                            current_reference_location.transpose(1, 2)[:, i : (i + num_track), :chunk_view-1],
                            data,
                            scales_relative_reference=scales_relative_current_step.transpose(1,2)[:, i : (i + num_track), 1:chunk_view] if self.enable_multiview_scale_align else None,
                            track_mask=data["track_valid_mask"].transpose(1, 2)[:, i : (i + num_track), :chunk_view-1],
                            query_movable_mask=data["query_movable_mask"][ :, i : (i + num_track)] if 'query_movable_mask' in data else None,
                        )  # B * n_view-1 * n_track * 2

                    else:
                        query_points_refined, reference_points_refined = data["query_points"][:, i : (i + num_track)], current_reference_location.transpose(1, 2)[:, i : (i + num_track), :chunk_view-1]
                        reference_points_refined = reference_points_refined.transpose(1,2)
                        conf = None # torch.ones(1,49,reference_points_refined.shape[1]).to(reference_points_refined.device)
                        
                    from pdb import set_trace as bb
                    # try:
                    #     data["query_movable_mask"][ :, i : (i + num_track)] = track_inlier_mask.unsqueeze(0)
                    # except:
                    #     bb()
                    # data["track_valid_mask"].transpose(1, 2)[:, i : (i + num_track), :chunk_view-1] = track_inlier_mask.unsqueeze(0).unsqueeze(-1).expand(-1, -1, chunk_view-1)
